 ...
 * 
 * This code is release under the LGPL license. You should have received a copy of the license in the LICENSE file. If you have not, see
...
/**
 * This class returns the hashes covering a certain bounding box. There are either 1,2 or 4 susch hashes, depending on the position of the bounding box on the
 * geohash grid.
 */
public class GeoHashBoundingBoxQuery implements GeoHashQuery, Serializable {
    ...
    private Set<GeoHash>      searchHashes     = new HashSet<GeoHash>();
    ...
    public GeoHashBoundingBoxQuery(BoundingBox bbox, int characterSize) {
        int fittingBits = characterSize * 5;
        WGS84Point center = bbox.getCenterPoint();
        GeoHash centerHash = GeoHash.withBitPrecision(center.getLatitude(), center.getLongitude(), fittingBits);
        if (hashFits(centerHash, bbox)) {
            addSearchHash(centerHash);
    ...
    public List<GeoHash> getSearchHashes() {
        return new ArrayList<GeoHash>(searchHashes);
    }
